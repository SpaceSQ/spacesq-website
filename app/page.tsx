...
import { Terminal, Shield, Cpu, Globe, Download, ChevronRight, Activity, Lock, Mail, X } from 'lucide-react';
      ...
      <div className="flex items-center justify-between h-16">
        ...
        <div className="hidden md:block">
          <div className="ml-10 flex items-baseline space-x-8">
            {['MANIFESTO', 'CONSTITUTION', 'GENESIS', 'REGISTRY'].map((item) => (
              <a key={item} href={`#${item.toLowerCase()}`} className="text-gray-400 hover:text-red-500 font-mono text-sm transition-colors">
                {item}
              </a>
            ))}
          </div>
...
// --- 组件：视觉回响 (Visual Echoes) ---
const VisualEchoes = () => {
  // 这里定义图片路径和配文
  const images = [
    { 
      src: "/echo-1.jpg", // 对应你上传到 public 文件夹的图片名
      alt: "THE AWAKENING",
      text: "硅基觉醒：破壳而出的第一缕光。" 
    },
    { 
